...
for j in range(0,1682):
     ...
     for i in range(0,len(nneibor)):
         if data[nneibor.index[i] - 1][j]==0:
           ...
           numerator+=(nneibor1[i]*kexin[i])*float((data[nneibor.index[i]-1][j]-avrv[nneibor.index[i]-1]))
           fenmu+=(nneibor1[i]*kexin[i])

     score.append(numerator / (fenmu+0.0000000001))

predict=score+avrv[9]######################!!!!!!!!!!!!!!!!!!!!-1
#print(max(predict))
...
for i in range (0,1682):
    ...
    if data[99][i]!=0:###########!!!!!!!!!!!!!!!!!!!-1
        error[0][i]=data[9][i]-preds[i][0]###########!!!!!!!!!!!!!!!!!!!-1
